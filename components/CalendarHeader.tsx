...
// changes size from one month to the next. Longest strings the two lines have
// to hold: 18 chars Gregorian, 25 Hijri ("28 Jumādá al-ākhirah 1448").
//
...
const GREGORIAN_MAX_CHARS = 18;
const HIJRI_MAX_CHARS = 25;
const BOLD_CHAR_WIDTH = 0.475; // em, measured against the system face
const SEMIBOLD_CHAR_WIDTH = 0.46;

// The mockup sets the Hijri line at just under half the Gregorian one (digit
...
const HIJRI_RATIO = 0.55;

const scaleType = (headerHeight: number, width: number) => {
  const usable = width - HORIZONTAL_PADDING * 2;
  const gregorian = Math.floor(
    Math.max(
      34,
      Math.min(
        52,
        headerHeight * 0.24,
        usable / (GREGORIAN_MAX_CHARS * BOLD_CHAR_WIDTH)
      )
    )
  );
  return {
    ...
    hijri: Math.floor(
      Math.max(
        ...
        Math.min(
          26,
          gregorian * HIJRI_RATIO,
          usable / (HIJRI_MAX_CHARS * SEMIBOLD_CHAR_WIDTH)
        )
      )
    ),
  };
      ...
      >
        ...
        <Text
          ...
          style={[
            TEXT_SHADOW,
            TEXT_BOUNDS,
            { fontSize: type.gregorian, lineHeight: type.gregorian * 1.15 },
          ]}
          numberOfLines={1}
        ...
        {/* Space is reserved even before the Hijri date resolves, so the
            header doesn't reflow when the calendar cache lands. */}
        <Text
          ...
          style={[
            ...
            TEXT_BOUNDS,
            {
              ...
              // Gregorian line at its larger size. Kept light enough to stay
              // legible over the darker prayer backgrounds.
              color: "rgba(255,255,255,0.72)",
              fontSize: type.hijri,
              lineHeight: type.hijri * 1.3,
            },
          ]}
          numberOfLines={1}
          adjustsFontSizeToFit
